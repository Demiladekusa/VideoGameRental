...
platforms = ["PlayStation", "Xbox", "Nintendo", "PC","steam","Wii"]
...
with open("Game_Infos.txt", "w") as file:
    file.write("ID Platform Genre Title Purchase Price £ Purchase Date\n")

    for game_id in range(1, 90):
        platform = random.choice(platforms)
        genre = random.choice(genres)
        title = random.choice(titles) + f"_{game_id}"
        purchase_price = random.randint(20, 80)
        purchase_date = random_date()

        line = f"{game_id} {platform} {genre} {title} {purchase_price} {purchase_date}\n"
        file.write(line)
